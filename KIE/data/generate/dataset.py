...
from KIE.data.preprocessing import generate_csv_for_training, generate_csv_for_evaluation

from torch.utils.data import Dataset
from torch.nn.utils.rnn import pad_sequence
from .variables import VOCAB, TEXT_MAX_LENGTH
...
class CustomDataset(Dataset):
    def __init__(self, new_dir: dict, class_dict=None, validation_args=None):
        """
        An abstract representation of a custom Dataset for receipts.
        ...
        if split not in splits:
            raise ValueError("The provided split is not supported. It must be one the {0}".format(splits))
 ...
 # collate_fn is used since the inputs are encoded and are of differernt lengths
 # randomly selecting batches of differnt input sizes will cause error.
 # collate_fn is used to pad the inputs to be of the same size tensor
